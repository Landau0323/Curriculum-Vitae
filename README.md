...
- 論文調査を行い、関連する先行研究をサーベイすることで、必要な技術を短期間でキャッチアップできる。
- 数理的な素養がある（物理の研究経験:5年）。
- 統計的手法・機械学習手法（ディープラーニングもそうでない手法も）全般、画像処理・画像認識の基礎的な知識
  - 画像採寸アプリ・ビジョンピッキングロボットシステム開発などにおいて、ディープラーニングおよび非ディープラーニング両方の画像認識手法を使った開発経験があります。（業務経験：3年弱）
- ライブラリやOSSを用いて機械学習手法を実装できる能力
  - ビジョンピッキングロボットシステム開発において、OSSをカスタマイズすることで、画像認識機能の実装を行った経験があります。  
- 自力で一から手法を実装できる能力
  - 現職で行った画像採寸アプリの開発時には、一からアルゴリズムを開発しました。（国内学会発表済、特許取得済）
...
- 製薬工程（震蘯）自動化ロボット（第2フェーズ）[開発]
  - 期間：2019年10月から11月
  - 概要: チューブ内を流れる液体中の粒子の動画から、粒子を画像認識により識別し、分離具合をスコアリングするソフトウェアを開発した。
  - 使用技術: python、semantic segmentation開発のマネージメント、データ分析（時系列データ、線形回帰） 
